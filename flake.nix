      ...
      #   nix build .#checks.x86_64-linux.verified-boot -L
        ...
        wizard = pkgs.testers.runNixOSTest {
          ...
          testScript = ''
            ...
            # The answers, in the order the wizard asks. The keyboard layout is
            # first and is confirmed by typing the sample string, which is the
            # step that stops a passphrase being entered on a layout nobody
            # checked.
            answers = "\\n".join([
                "de",                 # keyboard
                "hello-123",          # the layout check
                "de_DE.UTF-8",        # locale
                "Europe/Berlin",      # timezone
                "fw-wizard",          # hostname
                "operator",           # username
                "correcthorsebattery",# password
                "",                   # no SSH key
                "y",                  # configure the first NIC
                "wan",                # its zone
                "10.9.9.9/24",        # a static address, so the answer is visible
                "10.9.9.1",           # gateway
                "1",                  # install mode: single disk
                "1",                  # disk [1]
                "YES",                # the confirmation
            ]) + "\\n"
            machine.succeed(f"printf '%b' {answers!r} > /tmp/answers")

            with subtest("the wizard runs on a terminal and installs"):
                # `script` gives it a pty; without one the wizard refuses to
                # start and this would silently test the disk lister.
                machine.succeed(
                    "SENTINEL_INSTALL_SOURCE=${sentinelImageRaw} "
                    "script -qec 'sentinel install' /dev/null < /tmp/answers > /tmp/wizard.log 2>&1"
                )
                machine.succeed("udevadm settle")
                log = machine.succeed("cat /tmp/wizard.log")
                # The password must not survive in the scrollback of whatever
                # terminal or IPMI session did the install.
                assert "correcthorsebattery" not in log.split("Password:")[-1], log
