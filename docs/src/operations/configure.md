...
The same service that serves the console answers programs, on the same port.
Everything but `GET /api/v1/health`, `POST /api/v1/login` and the document
below sits behind a bearer token — the machine token in
`/var/lib/sentinel/api-token`, or the one `login` hands back for an account.

| Route | What |
|---|---|
| `GET /api/v1/openapi.json` | **The API as OpenAPI 3.1**, generated by the binary that serves it; `sentinel api --openapi` prints the same. Feed it to a client generator or an IDE. A test in the repository fails when a route is served and not in it, or in it and not served. |
| `GET` / `PUT /api/v1/config` | The whole configuration as JSON — read with every secret redacted, replaced through the same validate-apply-save path a `commit` takes. |
| `POST /api/v1/configure` | A script of `set` / `delete` / `commit` lines, run exactly as the CLI would; the answer is what the CLI would have printed. |
| `GET /api/v1/show/<path>` | An operational `show`, `ip/route` for `show ip route`. |
| `GET /api/v1/trace?in=…&proto=…&src=…&dst=…&port=…` | Where a packet would go and which rule decides it — see [Firewall & NAT](../handbook/firewall.md#asking-before-you-send-packet-trace). |
| `GET /api/v1/rule-hits`, `/api/v1/metrics/…`, `/metrics` | Which rules carry traffic, the history series, the Prometheus exposition. |
| `POST /api/v1/capture`, `POST /api/v1/clear/<path>` | A bounded packet capture; lifting run-time state a detector created. |
| `GET /api/v1/stack`, `/api/v1/stack/<member>/show/<path>` | The config-sync peers, and a `show` forwarded to one of them. |
