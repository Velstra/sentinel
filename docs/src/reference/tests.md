...
| `api` | the REST management API drives one config model end to end; `/health` is unauthenticated, the rest is not; and the web console — that it fetches nothing external, that every `/api/v1/…` path the page calls really answers, that a rule clicked into being is applied *and* persisted through the same commands the CLI runs, that a zone posture and a NAT entry are created and removed the same way, that a script with no `commit` changes nothing, that a refused command comes back as output rather than as a silent success, and that the stack refuses a member the appliance has no relationship with |
